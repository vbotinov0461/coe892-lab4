...
def mine_delete(mine_id: int):
    ...
    # if len(data["mines"]) == original_len:
    #     raise HTTPException(status_code=404, detail="Mine not found!")
    with open(mines_file, "w") as f:
        json.dump(data, f, indent=4)
...
def mine_create(body: MineInfo):
    ...
    # Add json record
    data["mines"].append([max_id + 1, body.serial, [body.x, body.y]])
    with open(mines_file, "w") as f:
        json.dump(data, f, indent=4)
    return max_id
    ...
    if os.path.exists(rover_dir):
        for file in os.listdir(rover_dir):
            try:
                ...
                continue
    filepath = os.path.join(rover_dir, f"rover_{max_id}.json")
    with open(filepath, "w") as f:
        json.dump({"rover": [max_id, "Not Started", [0, 0], body.instructions]}, f)
...
def rover_delete(rover_id: int):
    ...
    filepath = os.path.join(rover_dir, f"rover_{rover_id}")
    if os.path.exists(filepath):
        os.remove(filepath)
    else:
...
def rover_dispatch(rover_id: int):
    ...
    for c in data["rover"][3]:
        if (c == "L") or (c == "R"):
            facing = turn(c, facing)
        elif c == "M":
            # Check if we are leaving a mine
            boom = False
            map_data = get_json(map_file)
            if map_data["grid"][loc[1]][loc[0]] == 1:
                boom = True
            else:
                loc = move(loc, facing)
            if boom:
    ...
    # Mark as Finished
    data = get_json(filepath)
    if data["rover"][1] != "Eliminated":
        data["rover"][1] = "Finished"
    data["rover"][2] = loc
    with open(filepath, "w") as f:
        json.dump(data, f, indent=4)
    return data
